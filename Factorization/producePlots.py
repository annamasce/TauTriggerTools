...
argParser.add_argument('--WP',                    action='store',       default = 'mediumTauMVA',       choices=['vlooseTauMVA', 'looseTauMVA', 'mediumTauMVA', 'tightTauMVA','vtightTauMVA'])
argParser.add_argument('--var',                   action='store',       default = 'leadingpt') 
argParser.add_argument('--ptThreshold',           action='store',       default = '40') 

args = argParser.parse_args()

in_file = ROOT.TFile.Open("data/test/"+args.ptRegion+"/"+args.ptThreshold+"/tauTriggerFactorization2018-2D-v7.root")
WP = args.WP
tauDMs = ['dm0', 'dm1', 'dm10']
...
for h in histNames:
    ...
    destination = "/user/lwezenbe/private/CMSSW_10_2_17/src/TauTriggerTools/Factorization/Results/Factorization/HLT_DoubleMediumChargedIsoPFTauHPS35_Trk1_eta2p1_Reg_v/test/"+args.ptRegion+"/"+args.ptThreshold+"/"+args.var+'/'+WP+'_'+h
    if args.var == '2D': 
