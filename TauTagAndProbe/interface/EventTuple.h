...
#define EVENT_DATA_DITAU() \
    ...
    /* Basic tau variables */ \
    VAR(std::vector<UInt_t>, tau_sel) /* how tau was selected */ \
    VAR4(std::vector<Float_t>, tau_pt, tau_eta, tau_phi, tau_mass) /* 4-momentum of the tau */ \
    VAR(std::vector<Int_t>, tau_charge) /* tau charge */ \
    VAR(std::vector<Int_t>, tau_gen_match) /* matching of the tau with leptons on the generator level:
                                 Electron = 1, Muon = 2, TauElectron = 3, TauMuon = 4, Tau = 5, NoMatch = 6 */ \
    VAR(std::vector<Int_t>, tau_gen_charge) /* charge of the gen lepton that was matched with the tau */ \
    VAR4(std::vector<Float_t>, tau_gen_vis_pt, tau_gen_vis_eta, tau_gen_vis_phi, tau_gen_vis_mass) /* visible 4-momentum of the
                  gen lepton that was matched with the tau */ \
    VAR4(std::vector<Float_t>, tau_gen_rad_pt, tau_gen_rad_eta, tau_gen_rad_phi, tau_gen_rad_energy) /* visible 4-momentum of the
                  initial state radiation emmited by the gen tau */ \
    VAR4(std::vector<Int_t>, tau_gen_n_charged_hadrons, tau_gen_n_neutral_hadrons, tau_gen_n_gammas, tau_gen_n_gammas_rad) /*
    ...
    /* Tau ID variables */ \
    VAR(std::vector<Int_t>, tau_decayMode) /* tau decay mode */ \
    VAR(std::vector<Int_t>, tau_oldDecayModeFinding) /* tau passed the old decay mode finding requirements */ \
        TAU_IDS() \
    /* Tau transverse impact paramters.
       See cmssw/RecoTauTag/RecoTau/plugins/PFTauTransverseImpactParameters.cc for details */ \
    VAR(std::vector<Float_t>, tau_dxy) /* tau signed transverse impact parameter wrt to the primary vertex */ \
    VAR(std::vector<Float_t>, tau_dxy_error) /* uncertainty of the transverse impact parameter measurement */ \
    VAR(std::vector<Float_t>, tau_ip3d) /* tau signed 3D impact parameter wrt to the primary vertex */ \
    VAR(std::vector<Float_t>, tau_ip3d_error) /* uncertainty of the 3D impact parameter measurement */ \
    VAR(std::vector<Float_t>, tau_dz) /* tau dz of the leadChargedHadrCand wrt to the primary vertex */ \
    VAR(std::vector<Float_t>, tau_dz_error) /* uncertainty of the tau dz measurement */ \
    /* mu-tau variables */ \
    ...
    VAR(std::vector<Int_t>, l1Tau_hwQual) /* integer "hardware" quality value of the L1 tau */ \
        /**/
...
DECLARE_TREE(tau_trigger, Event, EventTuple, EVENT_DATA_DITAU, "events")
#undef VAR

#define VAR(type, name) ADD_DATA_TREE_BRANCH(name)
INITIALIZE_TREE(tau_trigger, EventTuple, EVENT_DATA_DITAU)
#undef VAR
#undef VAR2
#undef VAR3
#undef VAR4
